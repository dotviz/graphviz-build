...
{
    // Stack to simulate recursion - stores pairs of (node, from_node)
    typedef struct {
        Agnode_t *node;
        Agnode_t *from;
    } stack_item_t;
    
    // Estimate maximum stack size needed
    // Since we only follow tree edges and trees are acyclic,
    // worst case is visiting all nodes reachable through tree edges
    Agraph_t *g = agraphof(v);  // Get the graph containing this node
    int node_count = agnnodes(g);  // Total number of nodes in graph
    
    // Conservative estimate: allocate space for all nodes
    // In practice, we'll likely use much less since we only follow tree edges
    stack_item_t *stack = malloc(node_count * sizeof(stack_item_t));
    if (!stack) return best; // Handle allocation failure
    ...
    while (stack_size > 0) {
        ...
        subtree_t *ts = STsetFind(current);
        
        // Early termination if best has slack 0
        if (best && SLACK(best) == 0) {
            free(stack);
            return best;
        }
        ...
        // Process outgoing edges
        for (int i = 0; ND_out(current).list[i]; i++) {
            Agedge_t *e = ND_out(current).list[i];
            
            if (TREE_EDGE(e)) {
                if (aghead(e) == current_from) continue; // do not search back in tree
                
                // Push to stack (no need to check capacity since we pre-allocated enough)
        ...
        // Process incoming edges (mirror the above logic)
        for (int i = 0; ND_in(current).list[i]; i++) {
            Agedge_t *e = ND_in(current).list[i];
            
            if (TREE_EDGE(e)) {
                if (agtail(e) == current_from) continue;
